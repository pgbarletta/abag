...
if __name__ == '__main__':
    ...
    for pdb_idcode in pdb_list:
        ...
        # # abg_chain = tuple(interacting_chains[pdb_idcode])
        
        # # CDR
        # all_cdr_atoms = np.unique(list(itertools.chain(
        #     *[fila.cdr_atoms for index, fila in df_dataset.query(
        #     f"idcode == '{pdb_idcode}' and chainID in {abg_chain}").iterrows()])))
        # cnt_cdr_SC, cnt_cdr_BB = count_ONs(all_cdr_atoms, trj_in.topology, abg_chain)
        # # epitope
        # all_epitope_atoms = list(set(itertools.chain(
        #     *[fila.epitope_atoms for index, fila in df_dataset.query(
        #     f"idcode == '{pdb_idcode}' and chainID in {abg_chain}").iterrows()])))
        # cnt_epi_SC, cnt_epi_BB = count_ONs(all_epitope_atoms, trj_in.topology, abg_chain)
